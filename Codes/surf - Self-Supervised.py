...
num_iterations = 15  # 设置随机挑选的迭代次数

for i in range(num_iterations):
    # 重置随机种子，以确保每次迭代生成的测试集是完全独立的
    random.seed(i)
    # 独立地随机选择测试组
    test_subjects = random.sample(list(subject_id_list), 3)
    train_subjects = [subject_id for subject_id in subject_id_list if subject_id not in test_subjects]

    print('\nTest Subjects: ', test_subjects)
    print('Train Subjects: ', train_subjects)

    # 每次迭代都重新读取数据集并划分训练集和测试集
    train_loader, test_loader = get_data_loaders(df, train_subjects, test_subjects)
...
# 打印平均测试准确率和损失
print(f'Average test accuracy over {num_iterations} iterations: {np.mean(test_accs):.4f}')
print(f'Average test loss over {num_iterations} iterations: {np.mean(test_losses):.4f}')
