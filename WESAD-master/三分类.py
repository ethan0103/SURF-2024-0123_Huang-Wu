...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.mean(channel_attention * x, dim=2) * spatial_attention
        return x
...
class CNNLSTMModel(nn.Module):
    def __init__(self, lstm_hidden_dim=50, num_lstm_layers=1):
        ...
        self.cbam1 = CBAM(channels=16)
        self.pool = nn.MaxPool1d(kernel_size=2, stride=2, padding=0)
        self.conv2 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.cbam2 = CBAM(channels=32)

        conv_output_dim = calculate_conv_output_dim(10, 3, 1, 1)
        conv_output_dim = conv_output_dim // 4

        self.conv_output_size = 32 * conv_output_dim

        self.lstm = nn.LSTM(input_size=self.conv_output_size, hidden_size=lstm_hidden_dim, num_layers=num_lstm_layers,
                            batch_first=True)
...
for train_index, test_index in kf.split(df):
    ...
    model = CNNLSTMModel(lstm_hidden_dim=100, num_lstm_layers=1).to(device)
